...
Los recursos disponibles se pueden consultar en:
...
Recomendado:
- Nginx (Apache sirve también): Para servir la pagina en producción.
...
4. Ejecutar ``` python ./web/manage.py migrate ``` para ejecutar migraciones.

## 3.2. Tipos y Creacion de usuarios:
...
### 3.2.4. Usuario mantenimiento:

Este debe poder tener acceso al servidor en producción, el cual tiene las facultades de poder ejecutar comandos al archivo:
``` manage.py ```

Su rol es de instalar, ejecutar y mantener el servidor web (probablemente tu que estas leyendo esto) funcionando y actualizado.

Entre sus responsabilidades está:

* Subir y actualizar datos de hitos e edificios importantes.
* Crear usuarios
* Configurar sistemas
...
## 3.6. Carga y gestion de edificios o hitos del sistema:

### 3.6.1. Subir datos del IDE:
El servidor soporta la carga de archivos adquiridos de ``` http://www.ide.cl/descarga/capas ``` en formato DBF (SHP).

El script de carga pedirá al usuario ingresar nombres de columnas que son especificos para cada archivo DBF, para extraer los datos del archivo.
Lo cual requerirá una previa inspección de los datos, por parte del usuario mantenimiento, para obtener los nombres de las columnas que incluyen los datos basicos (requeridos): **nombre y coordenadas (latitud y longitud en decimales)**. Además, soporta los datos recomendados: **dirección y descripción**, que aparecen en las aplicaciones móbil y web.

El script se ejecuta mediante:
``` python ./web/manage.py import_dbf_landmarks ```
Y siguiendo las instrucciones en pantalla para configurar.

### 3.6.2. Gestionar (datos) de edificios o hitos del sistema:

**Usuario administrador** puede modificar los archivos mediante panel admin ingresando a URL:
```
<server-hostname>/admin
```
E introduciendo credenciales adminsitrador.

Accediendo a ```Landmarks ```y ```LandmarkTypes``` en el navegador se puede navegar por los datos actualizando o eliminando records.

**Usuario mantenimiento** puede acceder a estas configuraciones mediante dos caminos:
1. Crear usuario administrador mediante ``` manage.py createsuperuser```
2. Ingresando a base de datos mediante ``` psql ```, y gestionar datos por medio de SQL
...
## 4.2. [Desarrollo; Django] Ejecutar servidor desarrollo Django.
